'''
Unit tests for testing functions and small bits of logic for our skill
'''
...
def test_get_something():
    ...
    assert res == test_ref, "%s != %s" % (str(res), test_ref,)
...
def test_get_coolest():
    ...
    assert res in test_ref, "%s != %s" % (str(res), test_ref,)
