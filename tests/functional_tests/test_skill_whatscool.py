...
class TestFunctional:
    ...
    def setup_class(cls):
        ...
        cls.dir = os.path.join(package_root_dir, 'example_skills', 'alexa_skill_whatscool')
        ...
        cls.alexa_event_data = os.path.join(package_root_dir, 'example_skills',
                                            'alexa_skill_whatscool', 'lambda_test_data.json')
        # not generating random names on purpose
        # so that if things go wrong we don't pollute the account
        cls.function_name = 'whatscool_test'
        ...
        cls.ref_reply_list = [[None], [None], ["Lord of the rings", "Batman, the dark knight",
                                               "Indiana Jones"]]
    ...
    def test_replace_nested_dict_val(self):
